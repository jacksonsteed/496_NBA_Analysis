...
for(i in 2003:2020){
  ...
  for(j in unique_teams){
    this_team_games <- filter(this_year_games, HOME_TEAM_ID == j)
    wins <- sum(this_team_games$HOME_TEAM_WINS == 1)
    games <- sum(this_team_games$HOME_TEAM_WINS == 1 | this_team_games$HOME_TEAM_WINS == 0)
    pct <- wins/82
    team_winrates_by_season <- team_winrates_by_season %>% add_row(team_id = j, num_wins = wins, win_pct = pct, season = i)
  }
}
summary(team_winrates_by_season)
plot(team_winrates_by_season$win_pct)
plot(team_winrates_by_season$num_wins)
