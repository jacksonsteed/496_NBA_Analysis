...
final_no_zeroes <- final_dataset %>% filter(player_score != 0)

set.seed(123)
train_prop <- 0.7
train_size <- round(train_prop * nrow(final_no_zeroes))

train_ind <- sample(1:nrow(final_no_zeroes), size = train_size, replace = FALSE)
train_data <- final_no_zeroes[train_ind, ]
test_data <- final_no_zeroes[-train_ind, ]
...
X_train <- train_data[c("player_score")]
y_train <- train_data$PCT
# Train the model
model %>% fit(
...
predictions <- predict(model, as.matrix(test_data[c("player_score")]))

mse <- mean((predictions - test_data$PCT)^2)
